document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkBoard(){
        ...
        for(let y = 0; y < winningArrays.length; y++){
            const sqaure1 = squares[winningArrays[y][0]];
            const sqaure2 = squares[winningArrays[y][1]];
            const sqaure3 = squares[winningArrays[y][2]];
            const sqaure4 = squares[winningArrays[y][3]];
            
            // player-one win
            if(sqaure1.classList.contains('player-one') &&
            sqaure2.classList.contains('player-one') &&
            sqaure3.classList.contains('player-one') &&
            sqaure4.classList.contains('player-one')
            ) {
                result.innerHTML = 'Player One Wins!';
            }
            // player-two win 
            if(sqaure1.classList.contains('player-two') &&
            sqaure2.classList.contains('player-two') &&
            sqaure3.classList.contains('player-two') &&
            sqaure4.classList.contains('player-two')
            ) {
    ...
    for(let i = 0, len = squares.length; i < len; i++){
        squares[i].onclick = () => {
            // 현재 square을 taken 시 그 위의 sqaure 선택 가능
